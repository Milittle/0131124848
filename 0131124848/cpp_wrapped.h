...
info_pvf(const pstring& filename, const pstring& keyword)
{
    ...
    const char *p_filename_param;
    int len_filename;
    param_to_utf16(filename, filename_param, p_filename_param, len_filename);
    std::string keyword_param;
    const char *p_keyword_param;
    param_to_bytes(keyword, keyword_param, p_keyword_param);
...
int
shading(const pstring& shtype, double x0, double y0, double x1, double y1, double c1, double c2, double c3, double c4, const pstring& optlist)
{
    int retval = 0;
    ...
    PDFCPP_TRY {
	retval = m_PDFlib_api->PDF_shading(p, p_shtype_param, x0, y0, x1, y1, c1, c2, c3, c4, p_optlist_param);
    }
    PDFCPP_CATCH;
